...
func mockFindMatchStatusInProgressForTournamentFunc(ctx context.Context, id, tournamentID string) (*match.Match, errs.AppError) {
	matchMock := prototype.PrototypeMatch()
	matchMock.Status = model.MatchStatusInProgress
	return &matchMock, nil
}

func mockPostEventFunc(ctx context.Context, e event.Event) errs.AppError {
	return nil
}

func mockPostEventThrowFunc(ctx context.Context, e event.Event) errs.AppError {
	return errs.ErrRepoMockAction
}

func TestHandlePostMatchStart(t *testing.T) {

	goodReq := httptest.NewRequest(http.MethodPost, "/tournament/{id}/matches/{match_id}/events/start", nil)
	goodReq = mux.SetURLVars(goodReq, map[string]string{"id": "any", "match_id": "any"})

	missParamIDReq := httptest.NewRequest(http.MethodPost, "/tournament/{id}/matches/{match_id}/events/start", nil)
	missParamIDReq = mux.SetURLVars(missParamIDReq, map[string]string{"id": "", "match_id": "any"})

	throwReq := httptest.NewRequest(http.MethodPost, "/tournament/{id}/matches/{match_id}/events/start", nil)
	throwReq = mux.SetURLVars(throwReq, map[string]string{"id": "any", "match_id": "any"})

	goodReq2 := httptest.NewRequest(http.MethodPost, "/tournament/{id}/matches/{match_id}/events/start", nil)
	mux.SetURLVars(goodReq2, map[string]string{"id": "any", "match_id": "any"})

	missParamMatchIDReq := httptest.NewRequest(http.MethodPost, "/tournament/{id}/matches/{match_id}/events/start", nil)
	...
	testCases := []struct {
	...
	}{
		{
			...
			Name:                             "Should return 422 if match is already started",
			Request:                          throwReq,
			HandleGetTournamentFunc:          mockGetTournamentFunc,
	...
	for _, tc := range testCases {
		...
		HandlePostMatchStart(w, tc.Request)
