...
func HandlePostMatchHalftime(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if match.Status != model.MatchStatusInProgress {
		errs.HttpUnprocessableEntity(w, fmt.Sprintf("err: [%v]", "Match is not in progress to get halftime"))
		return
	}

	halftime := time.Now().Format("15:04")
	...
	data := map[string]string{
		"matchEventType": string(model.EventHalftime),
		"tournamentID":   tournament.ID,
		"matchID":        match.ID,
		"halftime":       halftime,
	}

	go kafka.Notify(ctx, data, model.ActionGameEvents, "Game Events - Halftime", model.KafkaTopicMatchEvents)
